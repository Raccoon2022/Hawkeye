...
void loop() {
  ...
  accel_x = msg.substring(keyPos[2]+1, keyPos[3]).toDouble();
  accel_y = msg.substring(keyPos[3]+1, keyPos[4]).toDouble();
  accel_z = msg.substring(keyPos[4]+1, keyPos[5]).toDouble();
  gyro_x = msg.substring(keyPos[5]+1, keyPos[6]).toDouble();
  gyro_y = msg.substring(keyPos[7]+1, keyPos[8]).toDouble();
  gyro_z = msg.substring(keyPos[9]+1, keyPos[10]).toDouble();
  magno_x = msg.substring(keyPos[10]+1, keyPos[11]).toDouble();
  magno_y = msg.substring(keyPos[11]+1, keyPos[12]).toDouble();
  magno_z = msg.substring(keyPos[13]+1, keyPos[14]).toDouble();
  altitude = msg.substring(keyPos[14]+1, keyPos[15]).toDouble();
  lat = msg.substring(keyPos[15]+1, keyPos[16]).toDouble();
  longi = msg.substring(keyPos[16]+1, keyPos[17]).toDouble();
  temperature = msg.substring(keyPos[17]+1, keyPos[18]).toFloat();
  pressure = msg.substring(keyPos[18]+1, keyPos[19]).toFloat();
  rssi = msg.substring(keyPos[19]+1, keyPos[20]);
  snr = msg.substring(keyPos[20]+1);

  Serial.println("a: "+accel_x+", "+accel_y+", "+accel_z+" gy: "+gyro_x+", "+gyro_y+", "
